...
def score_avg(user):
    # score가 있는 key만 추출. -> 해당 키의 값으로 평균 내기
    grade_field = (list(filter(lambda x: (x.find("score")) > 0, user)))
    avg = sum(list(user[x] for x in grade_field))/len(grade_field)
    return avg
